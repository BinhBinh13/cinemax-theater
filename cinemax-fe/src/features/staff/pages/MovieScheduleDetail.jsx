...
export default function MovieScheduleDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([getMovieById(movieId), getScheduleByMovieId(movieId), getRooms()])
      .then(([movieData, scheduleResponse, roomsResponse]) => {
        if (!mounted) return
        setMovie(movieData)
          ...
          <Modal.Body>
            ...
            <Form.Group className="mb-3">
              ...
              >
                ...
                {(rooms || [])
                  .filter((room) => room.status === 'ACTIVE' || room.id === Number(form.roomId))
                  .map((room) => (
                    <option key={room.id} value={room.id}>
                      {room.name} {room.status !== 'ACTIVE' ? `(${room.status})` : ''}
                    </option>
                  ))}
              </Form.Select>
            </Form.Group>
